...
#define ETHERNET_MIN_FRAME_LEN                  60
...
typedef struct TETHERNET_ADDRESS
{
    ...
    {
        return (EC_GETDWORD((EC_T_VOID*)b) % hashSize);
    }
...
#define EC_AL_ICMDHDR_SET_IRQ(p, wVal)          {(p)->__irq = EC_WORDSWAP((wVal));}
#else
#define EC_AL_ICMDHDR_GET_IRQ(p)                ((p)->__irq)
#define EC_AL_ICMDHDR_SET_IRQ(p, wVal)          {(p)->__irq = (wVal);}
#endif
...
EC_INLINE (EC_T_WORD EC_ICMDHDR_GET_IRQ(PETYPE_EC_CMD_HEADER p))
{
    return EC_GET_FRM_WORD(
...
typedef struct TEcCmdDesc
{
    ...
    EC_T_WORD               reserved2;
    EC_T_WORD               cmdSize;
    EC_T_WORD               imageOffs[2];
    EC_T_WORD               imageSize[2];
...
typedef struct _EC_T_MASTER_PROP_DESC
{
    ...
    EC_T_DWORD      dwRes1;
    EC_T_DWORD      dwRes2;
} EC_PACKED1 EC_T_MASTER_PROP_DESC;
...
EC_INLINE(EC_T_VOID EC_ECMBOXHDR_SET_LENGTH(ETHERCAT_MBOX_HEADER* p, EC_T_WORD wVal))
{
    EC_SET_FRM_WORD(
        ((EC_T_PBYTE)(&(p->__Length))), wVal
                      );
}
EC_INLINE(EC_T_WORD EC_ECMBOXHDR_GET_LENGTH(ETHERCAT_MBOX_HEADER* p))
{
    return EC_GET_FRM_WORD(
        ((EC_T_PBYTE)(&(p->__Length)))
                      );
}
...
} EC_PACKED1 EC_SDO_HDR, *PEC_SDO_HDR;
...
EC_INLINE(EC_T_WORD EC_ECSDOHDR_GET_INDEX(PEC_SDO_HDR p))
{
    return EC_GET_FRM_WORD(
        (((EC_T_PBYTE)p)+EC_ECSDOHDR_OFFS_INDEX)
                     );
}
EC_INLINE(EC_T_DWORD EC_ECSDOHDR_GET_SDODATA(PEC_SDO_HDR p))
{
    return EC_GET_FRM_DWORD(
        (((EC_T_PBYTE)p)+EC_ECSDOHDR_OFFS_SDODATA)
                      );
}
EC_INLINE(EC_T_VOID EC_ECSDOHDR_SET_INDEX(PEC_SDO_HDR p, EC_T_WORD wVal))
{
    EC_SET_FRM_WORD(
        (((EC_T_PBYTE)p)+EC_ECSDOHDR_OFFS_INDEX),
        wVal
...
EC_INLINE(EC_T_VOID EC_ECSDOHDR_SET_SDODATA(PEC_SDO_HDR p, EC_T_DWORD dwVal))
{
    EC_SET_FRM_DWORD(
        (((EC_T_PBYTE)p)+EC_ECSDOHDR_OFFS_SDODATA),
        dwVal
...
EC_INLINE(EC_T_PBYTE EC_ECSDOHDR_SDODATA(PEC_SDO_HDR p))
{
...
typedef struct TETHERCAT_SDO_INFO_ENTRY
{
    ...
    struct _t_sRes_sdoinfoentry
    {
        ...
        EC_T_BYTE   __ObjAccess     : 6;    /* bit0 = read; bit1 = write; bit2 = const. bit3 = �E�PRE-OP�E�E bit4 = �E�SAFE-OP�E�E bit5 = �E�OP�E�E */
#else
        EC_T_BYTE   __ObjAccess     : 6;    /* bit0 = read; bit1 = write; bit2 = const. bit3 = �E�PRE-OP�E�E bit4 = �E�SAFE-OP�E�E bit5 = �E�OP�E�E */
        EC_T_BYTE   __RxPdoMapping  : 1;    /* */
        EC_T_BYTE   __TxPdoMapping  : 1;    /* */        
...
/*  }; */
} EC_PACKED1 EC_FOE_HDR, *PEC_FOE_HDR;
...
EC_INLINE(EC_T_DWORD EC_ECFOEHDR_GET_PASSWORD(PEC_FOE_HDR p))
{
    return EC_GET_FRM_DWORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR)
                      );
}
EC_INLINE(EC_T_DWORD EC_ECFOEHDR_GET_PACKETNO(PEC_FOE_HDR p))
{
    return EC_GET_FRM_DWORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR)
        );
}
EC_INLINE(EC_T_DWORD EC_ECFOEHDR_GET_ERRORCODE(PEC_FOE_HDR p))
{
    return EC_GET_FRM_DWORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR)
        );
}
EC_INLINE(EC_T_WORD EC_ECFOEHDR_GET_STATUSDONE(PEC_FOE_HDR p))
{
    return EC_GET_FRM_WORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR)
        );
}
EC_INLINE(EC_T_WORD EC_ECFOEHDR_GET_STATUSENTIRE(PEC_FOE_HDR p))
{
    return EC_GET_FRM_WORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_ENTIRE)
        );
}
EC_INLINE(EC_T_VOID EC_ECFOEHDR_SET_ERRORCODE(PEC_FOE_HDR p, EC_T_DWORD dwVal))
{
    EC_SET_FRM_DWORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR),
        dwVal
...
EC_INLINE(EC_T_VOID EC_ECFOEHDR_SET_PACKETNO(PEC_FOE_HDR p, EC_T_DWORD dwVal))
{
    EC_SET_FRM_DWORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR),
        dwVal
...
EC_INLINE(EC_T_VOID EC_ECFOEHDR_SET_PASSWORD(PEC_FOE_HDR p, EC_T_DWORD dwVal))
{
    EC_SET_FRM_DWORD( 
        (((EC_T_PBYTE)p)+EC_ECFOEHDR_OFFS_UHDR),
        dwVal
...
} EC_PACKED1 EC_SOE_HDR, *PEC_SOE_HDR;
...
EC_INLINE(EC_T_WORD EC_ECSOEHDR_GET_IDN(PEC_SOE_HDR p))
{
    return EC_GET_FRM_WORD(
        (((EC_T_PBYTE)p)+EC_ECSOEHDR_OFFS_IDN)
                     );
}
EC_INLINE(EC_T_WORD EC_ECSOEHDR_GET_FRAGLEFT(PEC_SOE_HDR p))
{
    return EC_GET_FRM_WORD(
        (((EC_T_PBYTE)p)+EC_ECSOEHDR_OFFS_FRAGLEFT)
                     );
}
EC_INLINE(EC_T_DWORD EC_ECSOEHDR_GET_ERRORCODE(PEC_SOE_HDR p))
{
    return EC_GET_FRM_WORD( 
...
EC_INLINE(EC_T_VOID EC_ECSOEHDR_SET_IDN(PEC_SOE_HDR p, EC_T_WORD wVal))
{
    EC_SET_FRM_WORD(
        (((EC_T_PBYTE)p)+EC_ECSOEHDR_OFFS_IDN),
        wVal
...
EC_INLINE(EC_T_VOID EC_ECSOEHDR_SET_FRAGLEFT(PEC_SOE_HDR p, EC_T_WORD wVal))
{
    EC_SET_FRM_WORD(
        (((EC_T_PBYTE)p)+EC_ECSOEHDR_OFFS_FRAGLEFT),
        wVal
...
{ 
	EC_T_BYTE       oTargetNetId[6];/* destination NetID */
    EC_T_WORD       wTargetPort;    /* destination port */
	EC_T_BYTE       oSenderNetId[6];/* source NetID */
    EC_T_WORD	    wSenderPort;	/* source port */
    EC_T_WORD       wCmdId;			/* command id  */
	EC_T_WORD       wStateFlags;    /* state flags */
	EC_T_DWORD	    dwDataSize;     /* count bytes for actual command (excl. AmsHead) */
    EC_T_DWORD      dwErrorCode;    /* error code (only for responses) */
    EC_T_DWORD      dwAoeInvokeId;  /* will be cpied unchanged from request to response */
} EC_PACKED1 EC_AOE_HDR, *PEC_AOE_HDR;
...
} EC_PACKED1 ETHERCAT_AOE_READ_REQ_HEADER, *PETHERCAT_AOE_READ_REQ_HEADER;
...
} EC_PACKED1 ETHERCAT_AOE_READ_RES_HEADER, *PETHERCAT_AOE_READ_RES_HEADER;
...
} EC_PACKED1 ETHERCAT_AOE_WRITE_REQ_HEADER, *PETHERCAT_AOE_WRITE_REQ_HEADER;

/* Command ID 3 (Write response) */
typedef struct _ETHERCAT_AOE_WRITE_RES_HEADER
{ 
    EC_T_DWORD dwResult;
} EC_PACKED1 ETHERCAT_AOE_WRITE_RES_HEADER, *PETHERCAT_AOE_WRITE_RES_HEADER;
...
{ 
    ...
    EC_T_DWORD dwWriteLength;
    /* EC_T_BYTE  dwWriteData[1]; */
} EC_PACKED1 ETHERCAT_AOE_WRITECONTROL_REQ_HEADER, *PETHERCAT_AOE_WRITECONTROL_REQ_HEADER;

/* Command ID 5 (Write Control response) */
typedef struct _ETHERCAT_AOE_WRITECONTROL_RES_HEADER
{ 
    EC_T_DWORD dwResult;
} EC_PACKED1 ETHERCAT_AOE_WRITECONTROL_RES_HEADER, *PETHERCAT_AOE_WRITECONTROL_RES_HEADER;
...
{ 
    ...
    EC_T_DWORD dwWriteLength;
    /* EC_T_BYTE  dwWriteData[1]; */
} EC_PACKED1 ETHERCAT_AOE_READWRITE_REQ_HEADER, *PETHERCAT_AOE_READWRITE_REQ_HEADER;
...
}EC_PACKED1 EC_AOE_CMD_HDR, *PEC_AOE_CMD_HDR;
...
#define SIZEOF_EcMailboxCmdDescCoeData(p)   (EC_ECMBXCMDDESC_GET_DATALEN(((PEcMailboxCmdDesc)(p)))-sizeof(EC_SDO_HDR))
#define SIZEOF_EcMailboxCmdDescEoeData(p)   (EC_ECMBXCMDDESC_GET_DATALEN(((PEcMailboxCmdDesc)(p))))
#ifdef INCLUDE_SOE_SUPPORT
#define SIZEOF_EcMailboxCmdDescSoeData(p)   (EC_ECMBXCMDDESC_GET_DATALEN(((PEcMailboxCmdDesc)(p)))-sizeof(EC_SOE_HDR))
#endif
#define SIZEOF_EcMailboxCmdDescAoeData(p)   (EC_ECMBXCMDDESC_GET_DATALEN(((PEcMailboxCmdDesc)(p)))-sizeof(EC_AOE_HDR))
...
} EC_PACKED1 EC_T_IPADDR, *EC_PT_IPADDR;
